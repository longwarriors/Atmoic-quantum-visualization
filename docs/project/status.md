...
| React/Three.js 场景 | 生产构建通过；QVPC/1 parser（含 body 逐样本校验）、HTTP client、能力矩阵、zustand store、React/three 组件与测试套件自检的 vitest 单测（25 个 spec 文件、504 项）带强制覆盖率门槛，运行结果经 `assert-no-skips` 核对为零 skip、经 `assert-coverage-scope` 核对本次运行**解析后**的覆盖率配置、本次运行写出的报告所列的文件集与 `coverage-scope.json` 完全一致、且各模块重算出的覆盖率均达标；PR-8A 起覆盖范围为 `src/**` 下全部 `.ts`/`.tsx`，共 24 个门禁模块（`src/api/types.ts` 是唯一排除项，且“仅含类型”由守卫强制）；2pz、3dz² 浏览器视觉复核通过 | 视觉回归仍是人工检查（截图回归未进 CI）；主 bundle 1,219 kB（gzip 333 kB）尚待拆分 |
