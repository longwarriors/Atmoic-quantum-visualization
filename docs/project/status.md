...
- **fixture 而不是服务器**：`web/e2e/fixtures.ts` 把六份切片 payload 与两份 catalog 逐字节取自 `tests/fixtures/visual/`（`scripts/write_visual_fixtures.py` 写、`tests/test_visual_fixtures.py` 重建并比对，23 项），所以一次像素 diff 只能是关于渲染的，不会变成关于服务器今天返回了什么的争论。
