...
下表是路线选择指南，不是当前 capability matrix：
...
在 $\psi\ne0$ 的区域成立，但接近节点时可能奇异。对实波函数，余维一的节点超曲面可以把配置空间分成 nodal pockets；一般复波函数在三维空间的零集通常是余维二，不一定分割空间。高维采样必须报告链相关性、有效样本量，并在确实存在彼此分离的 pockets 时报告跨 pocket 混合，不能只展示漂亮点云。
