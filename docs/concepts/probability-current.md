...
$$
\mathbf v=\frac{\mathbf j}{\rho}
=\frac{m}{\mu r\sin\theta}\mathbf e_\phi
=\frac{m}{\mu}\frac{(-y,\,x,\,0)}{x^2+y^2}.
$$

所以每条流线都是**柱半径 $s$ 与高度 $z$ 均为常数的圆**，绕 $z$ 轴角速度为 $m/(\mu s^2)$，弧长 $2\pi s$ 后闭合。
...
M1 的含时叠加态会破坏这一对称性：流线不再闭合，播种也必须改为三维密度加权。
