...
/**
 * The modules coverage.exclude drops on the grounds that they are TYPE-ONLY:
 * excluded from the per-file *thresholds* (nothing to count), still scanned
 * for coverage-ignore pragmas on purpose -- a pointless pragma there is the
 * kind that gets copy-pasted into a gated file next -- and, more importantly,
 * held to that "type-only" claim by "keeps the modules coverage excludes as
 * type-only actually type-only" below, which parses each one and fails on any
 * top-level statement the compiler would emit.
 *
 * src/api/schema.gen.ts is generated by `npm run codegen` from
 * tests/fixtures/openapi.json. openapi-typescript emits interfaces and type
 * aliases (verified on this tree: transpiling the generated module yields the
 * banner comment and `export {}`, nothing else), which is exactly the claim
 * this set makes -- and if the generator is ever configured to emit runtime
 * code, its `enum` option being the obvious way, the type-only guard goes red
 * instead of the exclusion quietly becoming a hole. What keeps the file itself
 * honest is src/api/schema.gen.test.ts, which regenerates it from the fixture
 * and byte-compares.
 */
const TYPE_ONLY_EXCLUDED_SOURCES = new Set(['api/types.ts', 'api/schema.gen.ts'])

/** Coverage-gated sources, plus the type-only modules coverage excludes. */
const isPragmaScannedSource = (path: string): boolean =>
  isCoverageGatedSource(path) || TYPE_ONLY_EXCLUDED_SOURCES.has(path)
...
describe('scan scope', () => {
  ...
  it('pragma-scans the coverage-gated modules plus the type-only ones, and nothing else', () => {
    expect(pragmaScannedSources.map(toConfigPath)).toEqual(COVERAGE_SCOPE.pragmaScanned)
    // These are the files this scan covers that coverage gating does not:
    // excluded from the per-file thresholds, but not from the pragma scan
    // (see the comment on TYPE_ONLY_EXCLUDED_SOURCES above). Named one at a
    // time as well as through the set, so dropping one from the set fails
    // here on a path a reader recognises and not only on a manifest diff.
    for (const path of ['api/types.ts', 'api/schema.gen.ts']) {
      expect(isCoverageGatedSource(path), path).toBe(false)
      expect(isPragmaScannedSource(path), path).toBe(true)
    }
  })
  ...
  it('keeps coverage-scope.json sorted, duplicate-free, and a superset chain', () => {
    ...
    // Every gated module is pragma-scanned; these two are the extras, and any
    // third one is a scope decision that needs its own review. Both are
    // excluded from the thresholds as type-only and both are held to that
    // claim by the parse in "committed suite integrity" below.
    ...
    ).toEqual(['src/api/schema.gen.ts', 'src/api/types.ts'])
  })
...
describe('committed suite integrity', () => {
  ...
  it('keeps the modules coverage excludes as type-only actually type-only', () => {
    ...
    // src/api/types.ts and src/api/schema.gen.ts. The count is asserted, not
    // just "at least one", because this scan passes vacuously over an empty
    // list: a module dropped from pragmaScanned would stop being parsed here
    // AND stop being gated, silently.
    expect(typeOnly.length, 'no type-only module to check -- has the manifest changed?').toBe(2)
